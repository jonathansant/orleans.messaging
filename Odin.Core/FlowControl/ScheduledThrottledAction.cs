...
public class ScheduledThrottledActionFactory : IScheduledThrottledActionFactory
{
	/// <inheritdoc />
	public ScheduledThrottledAction Create(
		RegisterTimerDelegate registerTimer,
		Func<object[], Task> action,
		ILoggingContext loggingContext,
		Action<ScheduledThrottledActionOptions>? configure = null
	) => new(registerTimer, action, loggingContext, configure);
}
...
/// Similar to <see cref="ThrottledAction"/> however this is more intended for longer schedule and will trigger in a timer.
/// </summary>
[DebuggerDisplay(OdinDiagnostics.DebuggerDisplayStringFormat)]
