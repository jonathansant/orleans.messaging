...
public sealed class ConsumerGrain : Grain, IConsumerGrain
{
	...
	public ConsumerGrain(
		...
		IDigestingUtilityServiceFactory digestingServiceFactory,
	...
	public override async Task OnActivateAsync(CancellationToken cancellationToken)
	{
		...
		_emptyMessage = _runtimeOptionsService.GetEmptyMessage(_topicConfig.Name);
	...
	private async Task Consume()
	{
		...
		for (var i = 0; i < topicConfigBatchSize; i++)
		{
			...
			var (message, byteMessage) = await ToMessageWithFallback(consumeResult);
		...
		if (failedMessages.Count > 0)
			await HandleProcessingError(failedMessages.Values.Cast<Message<byte[]>>().ToList());
	}
	...
	private async Task<(Message Message, Message<byte[]> RawMessage)> ToMessageWithFallback(
	...
	private async Task<Message> ToMessage(
		ConsumeResult<byte[], byte[]> consumeResult,
	...
	private async Task SendToDlq(List<Message> batch)
		=> await batch.ForEachAsync(SendOneToDlq);

	private Task SendOneToDlq(Message message)
	{
	...
	private async Task Acknowledge(ImmutableList<Message> batch)
...
{
	public Message<byte[]> RawMessage { get; init; }
}
