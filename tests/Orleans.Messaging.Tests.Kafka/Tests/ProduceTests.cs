...
public class ProduceTests
{
	...
	public ProduceTests(KafkaClusterFixture fixture)
	{
	...
	public async Task Produce_SingleMessage_IsReceived()
	{
		...
		await _client.Produce<TestMessage>(Topic, messageKey, new("hello", "test"));
	...
	public async Task Produce_MultipleMessages_AllReceived()
	{
		...
		for (var i = 0; i < count; i++)
			await _client.Produce<TestMessage>(Topic, messageKey, new($"value-{i}", "batch"));
		...
		messages.Select(m => m.Payload.Value)
			.Should()
			.BeEquivalentTo(Enumerable.Range(0, count).Select(i => $"value-{i}"));
	}
	...
	public async Task Produce_MessageWithHeaders_HeadersPreserved()
	{
		...
		messages[0].Headers.Should().ContainKey("x-version").WhoseValue.Should().Be("2");
	}
